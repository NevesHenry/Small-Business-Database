...
<body>
    ...
    try {
        $sql = "SELECT BusinessID,BusinessAddress,BusinessName,CostAndFees,BusinessServices,NumOfCustomers FROM Business WHERE BusinessName LIKE '" . $_POST['bname'] . "%'";
        ...
        if ($row) {
            ...
            do {
                echo '<tr><td>' . $row['BusinessID'] . '</td><td>' . $row['BusinessAddress'] . '</td><td>' . $row['BusinessName'] . '</td><td>' . $row['CostAndFees'] . '</td>
                <td>' . $row['BusinessServices'] . '</td> <td>' . $row['NumOfCustomers'] . '</td></tr>';
